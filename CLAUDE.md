...
3. Use `WebSearch` to find real AI news from the last 24 hours only, spanning models/products,
...
4. Write the issue following the fixed structure (see below).
5. Save markdown to `ai-brew-YYYY-MM-DD.md` at repo root.
6. Write/overwrite `index.html` at repo root — a single self-contained HTML file (no external
...
7. Commit (`git add ai-brew-*.md index.html`) and push directly to `main`. Skip the commit only
...
8. Print a short summary (date, top headline, commit SHA) to the job log.
...
- Daily issues: `ai-brew-YYYY-MM-DD.md` at repo root — one per day, never overwritten, never
  deleted. `index.html` is the only file that gets overwritten each run (always reflects
  *today's* issue).
